...
<div class="series-show series-video-show">
	<div class="card-header series-video-title">
	    ...
	    <div class="col-sm-9">
        	<h1>{{ $video->name }}</h1>
        	<a href="/series/{{ $series->slug }}"><h3><i class="fa fa-{{ $series->font_awesome}}"></i> {{ $series->name }}</h3></a>
	    </div>
	</div>
	...
	<div class="video-jaundis-container">
		<div class="col-sm-9 video-jaundis">
			...
			<div class="card-jaundis video-listings">
				<div class="card">
					<div class="card-content hard-edges">
						<p>{{ $series->name }}</p>
					</div>
					@foreach ($series->videos()->orderBy('series_video.order', 'asc')->get() as $videoListing)
						<a href="/series/{{ $series->slug }}/{{ $video->id }}">
							<div class="card-action @if($videoListing->id == $video->id) current-video @endif video-listing">
								@if($videoListing->id == $video->id)
